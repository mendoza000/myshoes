...
import { AiOutlineHeart, AiFillHeart, AiOutlineEye, AiOutlineLine, AiOutlineShoppingCart } from "react-icons/ai";
  ...
  useEffect(() => {
    if (product) {
      const finalImage = product.secondaryPhotos.find(el => {
        return el.id === indexOfImage
      })
      console.log(finalImage,)
      setImage(finalImage)
    }
  }, [indexOfImage, product])
  ...
  return <div className='w-screen h-screen flex flex-col bg-background_main_l'>
    ...
    <section className='w-full h-[60vh] mb-4 flex items-center overflow-hidden ' >
      ...
      after:-translate-y-[50%] after:z-0 after:bg-center after:bg-cover after:contrast-100 '>
        ...
        <div className='flex flex-col h-full justify-between w-7 absolute text-xs top-0 left-2 z-[21]'>
        ...
        <div className='absolute bottom-3 left-0 w-full flex justify-end h-[70px] z-20'>
          {product.secondaryPhotos.map(photo => {
            return (
              <div key={photo.id} className='w-[70px] h-[70px] inline-block relative ml-1 cursor-pointer' onClick={() => { setIndexOfImage(photo.id) }}>
                {
                  image?.id !== photo.id &&
    ...
    ${isBuying ? ' pointer-events-auto translate-y-0' : ''} `} >
      <div className={`h-full w-[95%] rounded-t-3xl mt-auto ml-auto mr-auto flex flex-col justify-between bg-background_main_l`}>
        <div className='absolute top-3 w-1/3 border-2 border-[rgba(80,80,80,0.2)] left-[50%] -translate-x-1/2  ' />
        <h4 className='text-2xl text-fonts_main ml-auto mr-auto mt-7'>{product.name}</h4>
        <span className='m-auto mb-3'>Size {selectedSize}</span>
        <div className='grid grid-cols-2 w-full place-content-center mb-2'>
          <h4 className='text-2xl m-auto' > {Number.parseFloat(product.price * amount).toFixed(2)} </h4>
          <div className='m-auto'>
            <button onClick={handleDownAmount} className='rounded-md border-buttons_main border-2 px-2 self-center'>
              -
            </button>
            <span className='text-2xl mx-3'>{amount}</span>
            <button onClick={handleUpAmount} className='rounded-md bg-buttons_main border-buttons_main border-2 px-2 self-center'>
              +
            </button>
          </div>
        </div>
        <p className='text-fonts_secondary text-base tall:text-lg w-[90%] mx-auto mt-0 mb-4'>{product.description}</p>
        <div className='w-full flex justify-center items-center '>
          <button className='w-[45%] cursor-pointer bg-opacity-40 text-opacity-40 mr-2 pt-3 pb-3 mb-2 hover:bg-opacity-60 transition-colors
         duration-200 grid grid-cols-[30%,70%] place-content-center bg-buttons_main rounded-md'>
          ...
          <button className='w-[45%] cursor-pointer bg-opacity-40 text-opacity-40 ml-2 pt-3 pb-3 mb-2 hover:bg-opacity-60 transition-colors
