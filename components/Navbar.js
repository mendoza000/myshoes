...
export default function Navbar() {
  ...
  return <>
    <div className={`fixed top-1 left-1${!navShowing ? '' : ' opacity-0 '} transition-opacity`}>
      <CiGrid41 className='w-12 h-12' onClick={handleOpenNavbar} />
    </div>
    <nav className={`w-2/3 bg-background_main transition-transform duration-300 h-full fixed flex flex-col justify-between
    top-0 left-0 pointer-events-none -translate-x-full text-fonts_main font-semibold max-w-[300px] min-w-[220px]
    ${navShowing ? ' transform-none pointer-events-auto z-10 ' : ''}`} >
    ...
    <section className={`w-screen h-screen transition-opacity duration-300 pointer-events-none opacity-0 bg-black fixed 
