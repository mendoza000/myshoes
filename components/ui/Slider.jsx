...
const Slider = () => {
  ...
  return (
    <div className="flex relative gap-3 px-5 w-full h-56 overflow-x-hidden mt-7">
      <button className="grid place-content-center absolute z-20 bg-gradient-to-r from-[rgba(25,25,25,0.2)] to-transparent h-full w-20 top-0 left-0" onClick={handlePrevIndex}>
        <AiOutlineArrowLeft className="w-16 h-16 opacity-50" />
      </button>
      {
        shoes?.map(shoe => {
          return <div key={shoe.id} className={`transition-all absolute ${shoe.isActual ? 'scale-105 z-20 left-1/2 -translate-x-1/2' : 'scale-50 pointer-events-none '}
      ...
      <button className="grid place-content-center absolute z-20 bg-gradient-to-r to-[rgba(25,25,25,0.2)] from-transparent h-full w-20 top-0 right-0" onClick={handleNextIndex}>
        <AiOutlineArrowRight className="w-16 h-16 opacity-50" />
      </button>
    </div>
  );
};
